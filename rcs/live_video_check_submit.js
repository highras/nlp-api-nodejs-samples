...
let URI = "/api/v1/livevideo/check/submit";
...
function check(videoUrl, type, userId, callback) {
    var paramsJson = JSON.stringify({ "type": type, "userId": userId, "video": videoUrl });
...
check(videoUrl, 1, "12345678", function (result, error) {
    if (!error) {
        console.log(result);
    } else
        console.log(error);
